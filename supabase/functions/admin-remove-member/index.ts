// ============================================================================
// admin-remove-member — Supabase Edge Function
//
// Safe member removal + archive lifecycle for Team Manager. Originally built
// in PR #254 (Codex); adopted into main with an added `unarchive` mode so
// archived members can be cleanly re-enabled (archive bans the login — the
// old client-side status flip left the ban in place).
//
// Modes:
//   - inspect:          return linked activity counts only (drives the modal)
//   - archive:          mark member inactive/end-dated AND ban their login
//   - unarchive:        mark member active again AND lift the login ban
//   - permanent_delete: delete BOTH the auth user and the profile row.
//                       Only allowed when the member has zero linked
//                       activity, so old bookings/posts/tasks keep their
//                       attribution. (Deleting only the profile row is what
//                       used to strand auth accounts and cause "A user with
//                       this email address has already been registered".)
//
// ACCESS CONTROL
//   - verify_jwt: true; re-checks the caller is a team admin.
//   - The primary owner can never be removed; callers can't remove themselves.
// ============================================================================
...
type RemoveMode = "inspect" | "archive" | "unarchive" | "permanent_delete"
...
Deno.serve(async (req: Request) => {
  ...
  if (!["inspect", "archive", "unarchive", "permanent_delete"].includes(mode)) {
  ...
  if (callerProfile.id === memberId && mode !== "inspect") {
  ...
  if (mode === "unarchive") {
    const { error: unarchiveErr } = await admin
      ...
      .eq("id", memberId)
    if (unarchiveErr) return jsonResponse({ ok: false, error: unarchiveErr.message }, 400)

    let auth_warning: string | null = null
    try {
      const { error: unbanErr } = await admin.auth.admin.updateUserById(memberId, {
        ban_duration: "none",
      })
      if (unbanErr && !/not found/i.test(unbanErr.message)) auth_warning = unbanErr.message
    } catch (err) {
      auth_warning = err instanceof Error ? err.message : "Could not re-enable login"
    }

    return jsonResponse({ ok: true, action: "unarchived", member_id: memberId, auth_warning })
  }
