...
import { AlertCircle, Loader2, Mail, Phone, Shield, User } from 'lucide-react'
...
export default function MyTeam() {
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in">
      ...
      {teamQuery.isLoading && (
        <div className="bg-surface rounded-2xl border border-border p-10 flex items-center justify-center text-text-light">
      ...
      {teamQuery.error && (
        <div className="bg-surface rounded-2xl border border-border p-6 flex items-start gap-2 text-amber-300">
              ...
              <tbody>
                {rows.map((m) => {
                  ...
                  return (
                    <tr
                      key={m.id}
                      className={`border-b border-border/20 last:border-0 hover:bg-white/[0.02] transition-colors ${inactive ? 'opacity-50' : ''}`}
                    >
                      {/* Member: avatar + name */}
...
function HeaderCell({ children }: { children: React.ReactNode }) {
  return (
    <th className="px-5 py-3 text-left text-[10px] font-semibold text-text-light uppercase tracking-wider">
      {children}
    </th>
  )
}
