...
For fresh Claude sessions, run `.claude/commands/start-session.md`
before coding. It loads context, checks open PRs, checks drift, activates
the relevant guardrails, and summarizes the safe next move.
...
- **(1) Reorder** — `MyTasksCard` Active list wraps in dnd-kit. Each row gets a small `<GripVertical>` handle pinned to the left edge, hover-revealed so it doesn't fight the existing checkbox + click-to-open-detail UX. Persisted to `team_members.preferences.my_tasks_order` (jsonb subkey, debounced 500 ms write via existing `setUserPreference`) so the order follows the user across devices/browsers. Sort: saved order wins; brand-new tasks bubble to top until first drag; filtered-out tasks (priority/source/completed filters) keep their canonical position when toggled back on.
- **(2) Priority filter** — new shared `<PriorityToggle>` (Flame icon pill + tabular count) drops into the toolbar of all three task widgets: `MyTasksCard` + `AssignmentBoardBody` (which drives both `studio_tasks` and `team_board`). Reads the existing `is_required` boolean — user clarified: "simply use the already implemented 'priority' toggle as the thing the new filter toggle captures." Per-session state (no persistence — matches show-completed eye's behavior). Stacks with the existing Source filter (Assigned vs Self) on MyTasksCard.
- **(3) Admin reassign** — `AdminEditTasksModal`'s inline edit form now exposes an **Assigned to** dropdown picker right under Title. The `admin_update_assigned_task` RPC already supported `p_assigned_to` / `p_clear_assigned_to` (since 2026-05-07) — this PR is purely UI on top. Studio-scope tasks render a "not applicable" hint instead (RPC raises on those). Deactivated current assignees surface as an "(inactive)" fallback option so admins see what they're changing from.
- **No DB migration** — `preferences` jsonb column on `team_members` was added in `20260513140000_team_members_preferences_column.sql`; this PR just adds a new subkey. The `is_required` flag has been on `assigned_tasks` since `20260423103002_task_requests_is_required.sql`.
- **Next**: user originally queued Sessions polish #2 (Bookings page restructure with left-rail Bookings · Calendar · Clients tabs) after the task tweaks. Wait for them to confirm before starting that one. Other in-flight: Flywheel event ledger Phase 1 (PR #203 open), Google Calendar Phase 2A (user needs to reconnect Google + verify Push pending).

### 2026-05-17 Sessions polish #1 — Calendar delete bookings + recurring scope picker

- **`CLAUDE:`** per user direction: "(1) we need to be able to delete Calendar bookings on calendar view. both on the modal and by right clicking there should be an option to delete. We need to also have the question ask 'this and all future events?' or 'this event only' for recurring events that are cancelled. Do this first and add these next ones to Queue: (2) add calendar view also to the booking page..."
- **Two delete surfaces wired**: (a) Delete pill in `BookingDetailModal` footer — admin-only via `useAuth().isAdmin`, sits to the LEFT of Close/Edit (`mr-auto`) per the sitewide convention that destructive actions live on the far left so the gold primary action stays rightmost. (b) Right-click on any week-grid booking block in `Calendar.tsx` opens a small contextual menu (View details / Edit booking / Delete booking…). Both routes funnel into the same `DeleteBookingDialog` instance held at the page level.
- **Recurrence detection**: new `fetchSessionRecurrenceShape(sessionId)` helper in `adminSessions.ts` does a single-row lookup of `recurrence_spec` + `recurrence_parent_id` and returns `{isTemplate, isChild}`. `DeleteBookingDialog` calls it on mount so the caller never has to know in advance whether the row is recurring — the dialog shows the scope picker only when needed.
- **Scope picker UX**: single bookings get a Cancel · Delete confirm, no extra UI. Recurring bookings (template OR child) get two `ScopeOption` radio cards — "Just this event" vs "This and all future events" — Google-Calendar-style. Default selection is `one` so the dialog can confirm immediately without the user touching the radio.
- **New migration `20260517230000_admin_delete_recurring_sessions.sql`** (applied to prod via MCP) creates `admin_delete_future_sessions(p_session_id, p_cancel_note)` RPC. Key semantics: resolves `template_id` (self if `recurrence_spec NOT NULL`, else `recurrence_parent_id`), uses the clicked row's `session_date` as the cutoff, deletes every row in the series with `session_date >= cutoff`, AND explicitly nulls out the template's `recurrence_spec` BEFORE the delete so a race against `spawn_recurring_session_instances()` between the DELETE and the next cron tick can't spawn fresh children of a now-dead series. Returns `{deleted_count, deleted_google_event_ids[]}` so the client sweeps Google Calendar in a single best-effort pass. Degenerates safely on non-recurring rows.
- **Multi-row sync error aggregation**: `adminDeleteFutureSessions()` client helper loops over the returned google_event_ids calling `deleteSessionEventFromGoogleCalendar()` and aggregates errors into ONE warning string ("3 Google Calendar events failed to delete. First error: ...") so long recurring series with multiple Google events don't stack toasts.
- **`bookingLabel(b)` helper** on the Calendar page formats the prompt label ("Bridget · Tue May 20 · 2:00 PM"). Single `DeleteBookingDialog` instance handles deletes from either source surface. `onDelete` on `BookingDetailModal` closes the detail modal first so the two never visually overlap. Right-click context menu closes on outside-click + Escape.
- **Queued (Sessions polish #2)**: user wants to add a Calendar view to the booking page by restructuring `/sessions` around a Settings/Assign-style left-side rail with **Bookings · Calendar · Clients** tabs. They explicitly want a **preview-first** before deciding whether to fully merge Calendar into the Bookings menu item (and potentially drop the separate `/calendar` top-nav entry). User has a "task tweak list" coming next that may slot in ahead of polish #2 — don't auto-start until they confirm.
...
- **Important architectural nuance**: task surfaces use rich custom row components (`TaskRow`, `StudioTaskRow`) rather than `<table>`, so today every descriptor is export-only (no `render`). The shared module is structurally drift-proof anyway — when a future surface adds a structured table view (e.g. BusinessHealth analytics rollup), each descriptor just gets a `render` function and the same array drives both surfaces. The pattern handles both cases uniformly.
- **Per-member export UX**: `<ExportButtons>` sits next to the Add Task button in AssignAdmin's task pane header; filename + PDF title both incorporate the selected member's name so downloads land clearly named in the user's Downloads folder. Disabled when no member is selected.
- **Studio Tasks export UX**: `<ExportButtons>` in the action bar; `studio-tasks` filename. Studio pane already grouped tasks by physical space (Control Room / Studio A / Studio B), so the export includes the `Studio Space` column populated for those rows.
- **Also shipped today (separate)**: `docs/LEARNING_LOG.md` (commit `8886610`) — user-facing personal dev-concepts reference seeded with the GitHub Desktop loop, Git/Vercel/architecture/code concepts the user encountered during PR #194. Future sessions: when the user says "add this to my learning log," append at the top following the established shape (TL;DR · When · Why · Mental model).
- **Next**: Sessions/Bookings export still needs an admin-table surface scoped first (extend Hub? new `/admin/sessions` page?) — ASK the user before redesigning a page just to host the table. After that: BusinessHealth all-team task history rollup using the same `taskExportColumns`. Forum activity export still deferred until the `chat_*` schema lands. KPI graph export remains parked per user.
...
- **Flat yellow buttons + transparent social icons — 2026-05-06 (in flight, this PR).** Two follow-ups on PR #103. **(1) Flat bright yellow buttons.** Every `bg-gradient-to-b from-gold to-gold-muted text-black hover:brightness-105` button across the app (12 sites: Layout Clock In, Sessions Book a Session + Add Client, Dashboard Book a Session, MultiTaskCreateModal Send, AddFromTemplateModal Add, AdminEditTasksModal Save, AdminEditSessionsModal Save, TaskReassignRequestModal Approve, AssignedTaskBoards Request to take, shared.tsx SubmitBar) flattened to `bg-gold text-black hover:bg-gold-muted`. The gradient's darker-bottom `to-gold-muted` step was dulling the bright `#FFD60A` introduced in PR #103 rev5; flat reads punchier and matches the fintech reference. Marigold-tinted shadows replaced with neutral `rgba(0,0,0,0.08)` for a clean drop. **(2) Transparent social media icons.** `MemberHighlights.SocialStat` flipped from a solid 40×40 yellow tile + black glyph to TRANSPARENT background with the brand glyph rendered in `text-text` (deep contrast in both themes). The previous yellow square's hard edges cut into surrounding text per user report; transparent + dark glyph integrates cleanly with the count text beside it. Hover surface stays clickable via a `-mx-2 -my-1 px-2 py-1` negative-margin/padding trick that extends the hit area past the visible glyph. The hardcoded `SOCIAL_CHANNELS` count entries (Instagram 1240, TikTok 560, YouTube 95) stay intact from PR #79 — user explicitly asked us to preserve the manual count work. Verified: build clean. **`CLAUDE:`** authored.
- **PR #103 — Light-mode revamp — 2026-05-04 (merged `cbbb4e2`).** User reported the rev17 light mode (PR #79's final state) was "horrible" — six iterations to land a clean fintech-style theme. **rev1**: borders `#1c1c22` black ink → `#e5e5e7` soft grey; body slightly warmer; cards lifted lighter; widget-card shadow bumped. **rev2**: flipped to grey body + WHITE cards per user reference image (Apple-tone Toggle demo), text bumped to pure black. **rev3**: added a multi-tonal background wash (warm cream/peach top-left → near-white center → cool grey bottom-right) on `body`. **rev4**: moved the wash from `body` onto `.dashboard-shell` so it shows up INSIDE the rounded page area (was hidden behind shell's flat bg). **rev5**: swapped `--color-gold` from marigold `#f0a623` to bright pure yellow `#FFD60A` — cascades to every `bg-gold` use across the app (buttons, accent badges, social tiles). The rev10 token-split is preserved: `text-gold` override stays at deep amber `#7a510f` for readable text on white. **rev6**: wash spread bumped from 36% radius to 75-90% ellipse so it actually covers the whole page area (not tiny corner patches); `MemberHighlights` social-media tiles in `src/components/members/MemberHighlights.tsx` flipped from `bg-[var(--color-text)]` (black) + `text-[var(--color-bg)]` (light glyph) to `bg-gold` (bright yellow) + `text-black` by default — matches the fintech reference's bright-yellow CTA treatment. **Contrast scan** between rev2 and rev3 closed the gaps in PR #79's rev17 accent-shade overrides — Tailwind's `/N` opacity modifier compiles a separate class (e.g., `.text-amber-300/80` ≠ `.text-amber-300`), so each opacity variant in actual codebase use now has its own selector → 700-tier in light mode. Also added overrides for missing tiers (`text-amber-100/500`, `text-orange-*` — entire family because rev17 didn't have it — `text-cyan-200/500`, `text-red-600`, `text-sky-500`) and all `text-gold/40-90` opacity variants → deep amber `#7a510f`. Pulled `--color-text-light` darker (`#76767d` → `#3f3f46`) so meta lines pass AA contrast on the new white cards. Dark mode untouched (its own gold + violet gradient stays). **Out of scope (deferred follow-up)**: more granular polish on social media icons + any remaining muddy/orange CTAs the user calls out next round (filed for PR #104). **Verified**: `npm run build` clean; tested via Vercel preview through 6 iterations with visual feedback. **`CLAUDE:`** authored.
- **PR #102 — Studio +Add modal + recurrence picker + full Members-pane chrome parity — 2026-05-03 (merged `26225d8`).** Iteration on PR #101's Studio tab UX based on user feedback (multi-pass: first the modal swap + recurrence; then full chrome parity with the Members pane — Save as Template, Select-mode bulk delete, Templates ▾ dropdown). Four changes: (1) **Single top +Add button** consistent with the Members tab pattern — removed the per-section inline +Add composer from `<StudioTasksPane />`; replaced with a single "+Add task" button in a title row beneath the action bar that opens the existing `<MultiTaskCreateModal />` pre-toggled to Studio scope. (2) **`<StudioSpacePicker />` pills in studio mode** — `MultiTaskCreateModal`'s `<DraftRowCard />` swaps `FlywheelStagePicker` for a new room picker when `scope === 'studio'`. Pills: `[All]` `[Control Room]` `[Studio A]` `[Studio B]`. `[All]` maps to `studio_space=NULL` (the catch-all bucket; lands in the StudioTasksPane "(no space set)" section) — a single task can only be in one physical room, so `[All]` = "no specific room." Member rows still see the FlywheelStagePicker (unchanged). (3) **`<RecurrencePicker />` pills (studio scope only)** — new picker in studio mode: `[None]` `[Daily]` `[Weekly]` `[Monthly]`. Saves as `recurrence_spec = {frequency, interval: 1}`. Member rows don't see this picker yet (column accepts member rows too — UI extension is a 1-line change later). **Backend** (`MCP-APPLIED:` migration `studio_task_recurrence_spec`, captured at `supabase/migrations/20260503040000_studio_task_recurrence_spec.sql`): adds `recurrence_spec jsonb NULL` column to `assigned_tasks` with shape CHECK enforcing the frequency enum; `assign_custom_tasks_to_members` reads `recurrence_spec` from each task element on both branches (member + studio); `get_studio_assigned_tasks` JSON projection adds `recurrence_spec`. `ADVISOR-VERIFIED:` 0 ERRORS, 75 WARN (unchanged — both RPCs replaced in-place with same signatures). **Frontend types**: `AssignedTask.recurrence_spec?` field + new `AssignedTaskRecurrenceSpec` type (mirrors `task_requests.RecurrenceSpec` shape so the future scheduler can read both tables uniformly); `CustomTaskDraft.recurrence_spec?` field; normalizer extension. **UI**: `MultiTaskCreateModal` extended with optional `initialStudioSpace` prop (currently passed as `null` from StudioTasksPane — admin picks per row). Studio task rows in `StudioTasksPane` show a small gold `🔁 weekly` badge when recurrence is set. (5) **`/daily` Studio Tasks widget now sections by room** (added in third iteration after user reported the consumer-side widget was showing a stale empty-state and no per-room grouping). `StudioAssignedTasksCard` in `AssignedTaskBoards.tsx` accepts a new `sectionedByStudioSpace?: boolean` prop. When true, visible rows group under Control Room · Studio A · Studio B headers + a top "(no space set)" backfill bucket. Empty sections HIDDEN (widget is space-constrained, unlike the admin pane). Empty-state copy updated from stale "scope migration" wording to "Add room-tagged tasks from Assign → Studio." Per-section render reuses `<TeamTaskRow />` verbatim so all the existing pending-toggle / request-to-take overlay behaviour stays intact. (6) **Modal click-target bug fix.** Reported 2026-05-04: clicking into the +Add task title field and dragging to select text would sometimes mouseUp on the backdrop (cursor leaves the input bounds), triggering `onClick={onClose}` on the overlay and dismissing the modal mid-edit. Fix in `FloatingDetailModal.tsx`: track `mouseDown` target via a ref; only close if BOTH `mouseDown` AND the click landed on the backdrop. Applies to every modal using `FloatingDetailModal` (TaskDetailModal, MultiTaskCreateModal, RequestDetailModal, etc.) — single fix cascades across the whole modal system. (4) **Full Members-pane chrome parity** (added in second iteration after user pushed back on visual mismatch). Action bar at the top: `Save as Template · Select` on the left, `Templates ▾` on the right. Title row underneath: `Studio Tasks` + `N tasks · M complete` + `Add Task` button — visually identical to the Members pane's "All Tasks for {Member}" row. **Save as Template** wires to a new local `<SaveAsStudioTemplateModal />` that calls `createTaskTemplate` + loops `addTaskTemplateItem` over the current studio tasks (template is scope-agnostic; can be re-applied back to studio or to any member). **Select** mode toggles row checkboxes from gold (completion) to rose (selection); the action bar morphs to `N selected · Select all · Delete · Cancel` with a two-step inline confirm on Delete (mirrors PR #84's pattern). Per-row Edit + Trash hide in select mode; row gets a rose-tinted bg + ring when selected. **Templates ▾** dropdown lists the team's templates; clicking one fetches its items via `fetchTaskTemplateDetail` and pre-loads them as drafts in `MultiTaskCreateModal` (initialScope='studio') so the admin can review/edit (especially `studio_space`, which the template doesn't carry) before sending. New `initialDrafts?: CustomTaskDraft[]` prop on `MultiTaskCreateModal` powers the pre-load. **Out of scope (deferred follow-up)**: recurrence auto-recreate engine — today the spec is captured + displayed but no scheduler fires new task rows on the cadence; that lands in its own PR. **Verified**: `tsc --noEmit` clean; `npm run build` clean (2.93s); local preview only the expected no-auth Supabase fetch errors. **`CLAUDE:`** authored.
- **PR #101 — Studio tab on Assign + `studio_space` room sections — 2026-05-03 (merged `0d550f6`).** Opens a new admin surface: a Studio tab on the Assign page sidebar (peer-level alongside Members + Templates) that lists studio-scope tasks grouped by physical room. **Backend** (`MCP-APPLIED:` migration `studio_space_for_studio_tasks` + `studio_space_drop_old_admin_update_overload`, captured at `supabase/migrations/20260503030000_studio_space_for_studio_tasks.sql`): adds `studio_space text NULL` column to `assigned_tasks` with a CHECK constraint enforcing it's NULL OR `(scope='studio' AND value ∈ {Control Room, Studio A, Studio B})`. Distinct from the booking system's `StudioSpace` enum (`'Studio A' \| 'Studio B' \| 'Home Visit' \| 'Venue'`) — bookings can happen off-site, but studio TASKS are always in physical rooms. `assign_custom_tasks_to_members` extended to thread `studio_space` through the studio-scope INSERT branch; `get_studio_assigned_tasks` JSON projection adds `studio_space` and ORDER BY now sorts by space; `admin_update_assigned_task` extended with `p_studio_space` + `p_clear_studio_space` (old 8-param overload DROPPED to keep PostgREST resolution clean — same pattern as PR #97's approve_task_request consolidation). `ADVISOR-VERIFIED:` 0 ERRORS, 75 WARN. **Frontend types**: new `StudioSpace` type + `STUDIO_SPACES` const in `adminTasks.ts`; `AssignedTask.studio_space?` field in `types/assignments.ts` + the queries normalizer; `CustomTaskDraft` + `AdminUpdateTaskPayload` interfaces extended. **UI** — new self-contained component [StudioTasksPane.tsx](src/pages/admin/StudioTasksPane.tsx): fetches all studio tasks, groups by space into 3+1 sections (Control Room · Studio A · Studio B + an optional "(no space set)" backfill section that appears at the TOP only when there are unbacked rows so admins notice it first). Per-section: section header with task count + per-room +Add inline composer (single-line title, ⌘/Ctrl+Enter to send; auto-attaches the section's room tag). Per-row: completion checkbox + edit pencil + trash with two-step inline confirm. Realtime subscription on `assigned_tasks` filtered to `scope=eq.studio` so cross-session edits land instantly. Local `<StudioTaskEditModal />` mirrors the existing AssignAdmin SingleTaskEditModal shape but adds a Room picker (3 spaces + "(no space set)" option). [AssignAdmin.tsx](src/pages/admin/AssignAdmin.tsx): `view` state extended to `'member' \| 'templates' \| 'studio'`; URL hash extended (`#studio` joins `#templates` and `#<memberId>`); new Studio sidebar tab between Members and Templates with the same chrome pattern (Building2 icon, gold-ring active state, ChevronRight indicator). **Out of scope (deferred to follow-up)**: bringing `studio_space` sectioning to the `/daily` Studio Tasks widget (`AssignedTaskBoards.tsx`) — admin-create surface ships first, the consumer-side widget update lands in its own PR once we know the column populates cleanly in production. Existing studio tasks created before this PR have `studio_space=NULL` and surface in the "(no space set)" backfill section. **Verified**: `tsc --noEmit` clean; `npm run build` clean (2.89s); local preview only the expected no-auth Supabase fetch errors. **`CLAUDE:`** authored.
- **PR #100 — Uniform pending symbols + green approve buttons + clean edit-diff container — 2026-05-03 (merged `b31f4b7`).** Visual-consistency pass across the request triad surfaces. Three changes: (1) **Sitewide kind icons** — admin queue (`PendingTaskRequestsWidget`) + `RequestDetailModal` icon containers now use `Minus` / `Edit2` / `Plus` to match the member-side pending-row badges (was `Trash2` / `Edit2` / `UserCircle2`). (2) **Unified approve buttons** — both surfaces render every kind's approve trigger + confirm as the same emerald outline `Check Approve…` / `Check Confirm approve` (was kind-tinted: rose `Approve delete…`, orange `Apply edit…`, emerald `Approve…`). Approve is a positive/affirmative action regardless of what's being approved; the kind context is already conveyed by the request row's badge above. (3) **Clean edit-diff container** — `EditDiff` (admin queue) and `EditDiffBlock` (modal) atoms drop their `bg-orange-500/[0.08] ring-1 ring-orange-500/25` chrome so the title-change row reads as flat text. Field labels (`Title:`, `Description:`, etc.) and the Current → Proposed arrow stay orange as the kind signal. **Out of scope**: warning copy color (`"the task will be deleted permanently"`) stays kind-coded for safety; the recurrence pill on `kind='create'` rows keeps its gold styling — unrelated to edit. **Verified**: `tsc --noEmit` clean; `npm run build` clean (3.19s); local preview only the expected no-auth Supabase fetch errors. **`CLAUDE:`** authored.
- **PR #99 — Sitewide orange-for-edit + clickable admin queue rows + pending-row opacity fix — 2026-05-03 (merged `1ebaad3`).** Three follow-ups stitched after PR #98 sanity check. **(1)** The whole admin request row body in `PendingTaskRequestsWidget` becomes a single `role="button"` container instead of just the title text — fixes a regression where `kind='create'` rows couldn't open the modal because the title `<button>` shrunk to ~0px width inside a flex-wrap parent. Bigger click target as a side benefit; inline approve/decline sit in a separate row, unaffected. **(2)** Pending-row opacity moved off the row container onto the title text — `opacity-60` was being inherited multiplicatively by the badges, dimming them below the admin queue's badge brightness. Replaced with `text-text-muted` on the title + slightly lighter row bg (`bg-white/[0.025]` vs the active `bg-white/[0.018]`); badges now render at full saturation, matching the admin queue's visual weight. Same opacity fix applied to `PendingCreateRequestRow`. **(3)** Orange (was gold) is now the canonical edit color across all four request-triad surfaces: `PendingTaskRequestsWidget` (icon container, badge, EditDiff atom, approve confirm copy + buttons), `AdminApprovalLogWidget` ("Edited" tag + icon), `RequestDetailModal` (kind header chrome, EditDiffBlock atom, approve buttons, kind-aware confirm container bg), and `TaskDetailModal` (member-side Edit pill + composer tone — `ComposerShell` extended with `'orange'` tone alongside the existing `'gold'` and `'rose'`). **CSP/stale-bundle issues surfaced during preview testing** (pre-existing, unrelated to this PR — added 2026-05-01 in a Codex CSP session): `vercel.live/_next-live/feedback/feedback.js` blocked by CSP (Vercel preview comment widget); inline recovery script's sha256 hash in CSP is stale; stale browser cache of `index.html` references chunk filenames that don't exist after a deploy → MIME mismatch crash. **Filed as a follow-up chip**: CSP whitelist for `vercel.live` + recovery-script hash regeneration + `vite:preloadError` global handler that auto-reloads on stale chunk. **`CLAUDE:`** authored.
- **PR #98 — RequestDetailModal + member cancel — 2026-05-03 (merged `23d5cce`).** Two user-requested follow-ups stitched into one PR after the request triad shipped in PR #97. **(1) Admin click-into-request modal** — clicking a pending request title in PendingTaskRequestsWidget opens a new [`RequestDetailModal`](src/components/admin/assign/RequestDetailModal.tsx). The row's title is now a button; the inline Approve/Decline still work for one-click flow, the modal is the "more details" path. Modal reuses `FloatingDetailModal` (PR #86 portal-to-document.body fix → centers correctly inside transformed/backdrop-filtered ancestors). Body is kind-aware: `kind='create'` shows a meta grid (due / stage / recurrence / required); `kind='edit'` shows a larger `EditDiffBlock` per-field Current → Proposed table; `kind='delete'` shows the reason. Common: requester + when, kind badge (rose/gold/amber), and a description/reason section. Footer surfaces both Decline + Approve with their own two-step inline confirm (decline takes optional note; approve shows FlywheelStagePicker only on create; toast copy is kind-aware: `Edit applied — "X" updated for Y.` etc). The modal owns its own approve/reject mutations + invalidation, so it stays self-contained and could be opened from any surface (notifications panel, member's history view) without changing parent state. **(2) Member cancel** for any of their pending task_requests (kinds: create / edit / delete). New `cancel_my_task_request(p_request_id)` RPC (`MCP-APPLIED:` migration `task_request_cancel_and_status_cancelled`, captured at `supabase/migrations/20260503020000_task_request_cancel_and_status_cancelled.sql`): caller-only (`requester_id = auth.uid()`); requires `status='pending'`; sets `status='cancelled'` + `reviewer_id`/`reviewed_at` so the row appears in `admin_recent_approvals`; notifies all team admins via a new `task_request_cancelled` notification. EXECUTE locked to authenticated. `task_requests_status_check` extended to admit `'cancelled'`; `admin_recent_approvals` WHERE clause now includes `'cancelled'`. `ADVISOR-VERIFIED:` 0 ERRORS, 74 WARN (+1 from the new SECURITY DEFINER RPC, by-design). **Frontend cancel UX** mirrors PR #84's bulk-confirm pattern: hover-X on each pending row → first click morphs to "Cancel request? [Cancel] [Keep]" inline pill → confirm fires `cancelMyTaskRequest`. Auto-resets after 4s. Wired in two places in [MyTasksCard.tsx](src/components/tasks/MyTasksCard.tsx): (a) `AssignedTaskRow` for `kind='delete'`/`'edit'` rows — `PendingMeta` extended with optional `requestId` so the row component can call cancel without re-fetching; (b) `PendingCreateRequestRow` for `kind='create'` (in-flight new-task requests, which have no underlying assigned_task). Cancel button HIDDEN for `kind='transfer'` rows since transfers live in the sibling `task_reassign_requests` table — separate cancel flow ships in a future PR. [`AdminApprovalLogWidget.tsx`](src/components/admin/assign/AdminApprovalLogWidget.tsx) gains a "Cancelled" variant: muted Ban icon + uppercase "Cancelled" tag + `opacity-70` row dim, distinct from rejected (rose X) and approved-* (kind-specific). `RecentApprovalRow.status` type extended to `'approved' | 'rejected' | 'cancelled'`. **Helper**: new `cancelMyTaskRequest(requestId)` in [`taskRequests.ts`](src/lib/queries/taskRequests.ts); `TaskRequestStatus` type extended. **Verified**: `tsc --noEmit` clean; `npm run build` clean (2.74s); local preview only the expected no-auth Supabase fetch errors (no React/runtime errors from this PR's code). **Out of scope** (deferred): cancel for transfer requests (`task_reassign_requests` is its own table; same UX shape can land separately); admin-side undo for an approved request (intentional — once approved, the action is committed and only a reverse action — re-create, re-edit — should restore state). **Pattern note**: this PR ships the second piece of "kind branching" convergence — `RequestDetailModal` is now the single source of truth for "show me the full details + act on it" across all 3 task_request kinds, mirroring how `PendingTaskRequestsWidget`'s inline rows already converged in PR #97. Adding a 5th/6th kind in the future is now a 2-line change in each of: the queue widget icon/badge map, the modal's kind-aware body section, and the approval log's status icon. **`CLAUDE:`** authored.
- **PR #97 — Member request-to-edit flow — 2026-05-03 (merged `08dc7c3`).** Closes the Delete + Transfer + Edit triad started in PRs #82 / #84 / #85. Member opens one of their own active member-scope tasks via TaskDetailModal, taps the new gold `✎ Edit` pill (between Delete + Transfer); composer pre-fills 4 inputs (title, description, due date, flywheel stage select) with the task's current values plus an optional Reason; only changed fields go in `proposed`; admin sees the request in PendingTaskRequestsWidget with a tight Current → Proposed diff block; one-click "Apply edit" approves and the server updates the target task. **Backend** (`MCP-APPLIED:` migration `task_edit_request_rpcs`, captured at `supabase/migrations/20260503010000_task_edit_request_rpcs.sql`): (1) `task_requests.proposed jsonb` column + CHECK enforcing it's set ⟺ `kind='edit'`. (2) `submit_task_edit_request(p_task_id, p_proposed, p_reason DEFAULT NULL)` — SECURITY DEFINER; member-only (no admin gate); team-scoped lookup; `scope='member'` + `assigned_to=caller` required; filters proposed to allowed keys (`title`, `description`, `category`, `due_date`); per-key type validation (title can't be cleared; due_date must be parseable YYYY-MM-DD); inserts the row + notifies all team admins. EXECUTE locked to authenticated. (3) `approve_task_request` extended with `'edit'` branch — applies COALESCE/CASE updates to target task: absent key = unchanged, explicit null/empty = clear (description/category/due_date only). Returns `{updated_count}`. Notification body kind-aware ("Your edit request was approved"). (4) `get_pending_task_requests` + `get_my_task_requests` extended to surface `proposed`; pending ALSO returns a `current` snapshot of the target's editable fields so the diff renders without a second round-trip per row. `ADVISOR-VERIFIED:` 0 ERRORS, 73 WARN (was 72 — one expected by-design `authenticated_security_definer_function_executable` from the new SECURITY DEFINER RPC; the trigger function from PR #90 stays out of advisor since it's REVOKED from PUBLIC/anon/authenticated). **Frontend helper** ([taskRequests.ts](src/lib/queries/taskRequests.ts)): new `ProposedTaskEdit` + `CurrentTaskSnapshot` types; `PendingTaskRequest` + `MyTaskRequest` interfaces gain `proposed`/`current`; new `submitTaskEditRequest(taskId, proposed, reason?)` helper. **UI**: (a) [TaskDetailModal.tsx](src/components/tasks/TaskDetailModal.tsx) — third gold pill `✎ Edit` between Delete + Transfer (canRequest gate same as the other two: own active member-scope tasks); compose state machine extended to `'edit'`; composer reuses `<ComposerShell tone="gold" />` (single source of truth for the chrome) with body slots: title text input + description textarea + a 2-col grid of `<input type="date">` + flywheel stage `<select>` + reason text input; send disabled when nothing changed (computed via `useMemo` diff against task fields); Send label flips to "No changes" when empty; ⌘/Ctrl+Enter shortcut wired in the existing keydown handler. (b) [PendingTaskRequestsWidget.tsx](src/components/admin/assign/PendingTaskRequestsWidget.tsx) — `kind` discriminator (`isDelete` / `isEdit` / create-fallback) drives icon (Trash2 / Edit2 / UserCircle2), badge label ("Delete" / "Edit" / "New task"), and badge color (rose / gold / amber). New `<EditDiff />` atom renders for `kind='edit'` rows: gold-tinted row per changed field with `Field: <current> → <proposed>` and italic "none" placeholder for null/empty values. Approve confirm path skips the FlywheelStagePicker on edit (the proposed payload itself can rewrite category) and shows a gold "Apply edit" button + descriptive line. Approve toast kind-aware (`Edit applied — "X" updated for Y.`). (c) [AdminApprovalLogWidget.tsx](src/components/admin/assign/AdminApprovalLogWidget.tsx) — gold `Edit2` icon + small uppercase "Edited" tag on approved-edit rows, distinct from approved-create (emerald check), approved-delete (rose Trash2), and rejected (rose X). (d) [MyTasksCard.tsx](src/components/tasks/MyTasksCard.tsx) — `PendingKind` extended to `'edit'`; `pendingByTask` Map now picks up edit-kind rows from `myRequests`; pending row swap shows gold "Awaiting admin to edit" badge with `Edit2` icon. **Verified**: `tsc --noEmit` clean; `npm run build` clean (3.12s); local preview only shows the expected no-auth Supabase fetch errors (no React/runtime errors from this PR's code). **Out of scope (deferred)**: edit-rejection feedback view (member can already see reviewer_note via existing `My Requests` view); admin-side preview of what the task will look like after the edit (the diff already gives field-level info — full preview is overkill for a 4-field surface). **Pattern note**: this PR converged the "kind branching" idiom across all three queue widgets, so adding a fifth or sixth kind in the future is now a 2-line change per widget. **`CLAUDE:`** authored.
- **PR #96 — Templates as inline sidebar tab — 2026-05-03 (merged `d5a541d`).** Assign-page UX rework, second attempt. After scratching the 3-column rework (PR #95 — closed unmerged because the layout felt confusing on preview), the user redirected: Templates should behave like a member tab — clicking it swaps the main pane in place instead of routing to `/admin/template-library`. **TemplateLibrary.tsx** ([src/pages/admin/TemplateLibrary.tsx](src/pages/admin/TemplateLibrary.tsx)): new optional `embedded?: boolean` prop. When set, drops the full-page chrome (`PageHeader`, `useDocumentTitle`-style effect, `max-w-[1400px] mx-auto` wrapper) for a slim inline header bar (Layers icon + "Templates" + count badge + filled-tonal "+ New Template" on the right, separated by `border-b border-border/60`); tightens the card grid `grid-cols-2 md:grid-cols-3 lg:grid-cols-4` → `grid-cols-2 lg:grid-cols-3` so cards don't squish in the narrower Assign main pane. Same React Query cache key (`taskTemplateKeys.library`), same modals (`TemplatePreviewModal`, `TemplateEditorModal`) — embedded + standalone mounts stay in sync. **AssignAdmin** ([src/pages/admin/AssignAdmin.tsx](src/pages/admin/AssignAdmin.tsx)): replaces the Templates `<Link>` with a tab button. New `view: 'member' \| 'templates'` state; URL hash mirrors the selection (`#templates` or `#<memberId>`) so refresh + deep links stay put. New `selectMember(id)` helper auto-flips view back to `'member'` when a member tab is clicked. Templates-tab active state matches the member tabs visually (gold `ring-1 ring-gold/30` + chevron + bold label). Standalone `/admin/template-library` route still works for deep links. **Pattern reusable**: future sidebar tabs can mirror the same `view:` state-machine + URL-hash pattern. **PR #95 (scratched)**: 3-column rework — sidebar (260px) + main Assign (432px) + LogsCarousel (432px) all 696px tall with widget-card chrome; built end-to-end and Vercel-deployed for preview, but user found the layout confusing and closed the PR before merge. Local rework commits abandoned in branch `claude/assign-page-rework-preview` (closed PR #95). Remote branch deletion was permission-denied at the push layer; harmless because the PR is closed and the branch has no merge target. **Verified**: build clean (2.69s); preview at 1440×900 — Templates tab swaps main pane in place, gold ring on active, URL hash updates to `#templates`, standalone route still loads with own document title + PageHeader. **`CLAUDE:`** authored.
- **PR #94 — Removed empty "Settings for Tasks" button on the Assign page — 2026-05-02 (merged `f717c1e`).** User: "we need to get rid of the empty 'settings for tasks' button next to 'save as templates'. just delete it." Single-file change in [AssignAdmin.tsx](src/pages/admin/AssignAdmin.tsx) — the placeholder button had no handler and no follow-up scope. Trivial cleanup. **`CLAUDE:`** authored.
- **PR #90 — Assign delete reliability (Codex `codex/assign-delete-debug`) — 2026-05-03 (merged `740667d`).** Closes the user-reported "Assign tasks won't actually delete" bug. **3 migrations applied via MCP and verified before merge**: (1) `assigned_tasks_team_id_guard` — backfill NULL `team_id` from assignee or assignment-batch admin chain + `BEFORE INSERT OR UPDATE OF assigned_to, recipient_assignment_id, team_id` trigger calling `fill_assigned_task_team_id()` (SECURITY DEFINER, REVOKED from PUBLIC/anon/authenticated; only the trigger subsystem invokes); fallback chain assignee.team_id → batch_admin.team_id → `get_my_team_id()`. (2) `admin_delete_assigned_tasks_return_ids` — RPC now returns `{deleted_count, deleted_ids}` + adds NULL-team rescue branch (deletes legacy NULL-team rows when team ownership is provable via assignee or batch admin); team isolation preserved. (3) `cleanup_stuck_assign_test_tasks` — one-time DELETE of three specific stuck rows by title+due_date. **Frontend** ([adminTasks.ts](src/lib/queries/adminTasks.ts)): helper returns `{deleted_count, deleted_ids}` with safe defaults so the UI degrades gracefully if the new RPC shape isn't live. **UI** ([AssignAdmin.tsx](src/pages/admin/AssignAdmin.tsx)): mutation passes `{requestedIds, result}`; optimistic remove only fires for confirmed-deleted ids; toast logic honest (0/partial/full); select mode resets only on full success. `ADVISOR-VERIFIED:` 0 ERRORS post-apply. All 7 `INSERT INTO public.assigned_tasks` paths reviewed — 6 don't set team_id explicitly (now covered by the trigger); 1 already does (`assign_rpcs_set_team_id`, unchanged). **Operational caveat memorialized in PR description**: Vercel green ≠ Supabase migrations applied. Apply via MCP (or surface as a manual ops step) before merging any PR with new migration files. Authored by **Codex**; reviewed + applied + verified + PR description updated by **Claude**.
- **PR #89 — Sticky header fix (`overflow: hidden` → `overflow: clip`) — 2026-05-02 (merged `8dfa922`).** User: "the header no longer floats. it gets lost on scrolling." `.dashboard-shell` has had `overflow: hidden` since 2026-04-18 (commit `ec4bbda4`); that puts the sticky `<header>` inside the shell's scroll context, but the shell itself never actually scrolls (just clips), so the header had nothing to stick to. Bug technically existed since April 18 but only became visible NOW because the PR #88 pending section made pages tall enough to actually scroll on shorter viewports. Fix: `overflow: clip` on `.dashboard-shell` — same visual rounded-corner clipping but does NOT establish a scroll context. Browser support: Chrome 90+ / Firefox 81+ / Safari 16+ (~99%+). Verified at 1280×600: header at y=21 before scroll, sticks at y=0 after scrolling 491px down; `getComputedStyle(.dashboard-shell).overflow === 'clip'`. **`CLAUDE:`** authored.
- **PR #88 — MyTasksCard pending section + divider — 2026-05-02 (merged `4be99e3`).** User wanted visual identifier for in-flight delete / transfer / new-task requests. Backend (`MCP-APPLIED:` `reassign_request_direction_aware_queries`): replaced `get_my_incoming_reassign_requests` with direction-aware filter (was returning my outgoing transfer rows too because the existing WHERE clause keyed only on `current_assignee_id = caller`); new `get_my_outgoing_pending_reassign_requests()`. Frontend: `OutgoingReassignRequest` type, `fetchMyOutgoingPendingReassignRequests()`, `taskReassignKeys.outgoing()`. UI ([MyTasksCard.tsx](src/components/tasks/MyTasksCard.tsx)): full restructure — splits visible tasks into active vs pending; PENDING divider (Hourglass + amber caps); pending rows at opacity-60 with status badge ("Awaiting <teammate>" / "Awaiting admin to delete"); pending NEW task requests render as their own `<PendingCreateRequestRow />` variant; pending tasks suppress the Submit-Completed queue toggle to avoid race with admin/peer decision; realtime subscription extended to `task_requests` + `task_reassign_requests` so the section auto-promotes back to active when admin/peer declines. **`CLAUDE:`** authored.
- **PR #87 — Realtime sync for task surfaces — 2026-05-02 (merged `a5a9fe2`).** User-reported regression after PR #85: admin deleted in one browser, member's My Tasks in another browser still listed it; clicking → "Ask admin to delete" → server returned `task not found`. Root cause: the request-flow tables weren't in `supabase_realtime` publication, so the existing subscription in MyTasksCard never received cross-session events. Fix: `MCP-APPLIED:` `realtime_task_sync` — `REPLICA IDENTITY FULL` on `assigned_tasks` / `assignment_notifications` / `task_requests` / `task_reassign_requests` (so DELETE events carry full row data — required for client filters like `assigned_to=eq.<user-id>` to match) + `ALTER PUBLICATION supabase_realtime ADD TABLE` for each (idempotent via DO blocks). Wired missing realtime subscriptions on AssignedTaskBoards, AssignAdmin, PendingTaskRequestsWidget. Defensive `handleStaleTaskError` in TaskDetailModal: when server returns "task not found" (rare cache-race window), surfaces friendly `"This task was already removed by an admin."` toast + invalidates caches + closes modal. SQL verify: all 4 tables show `rep_id=full` and `in_pub=true`. **`CLAUDE:`** authored.
- **PR #86 — `FloatingDetailModal` portals to `document.body` — 2026-05-02 (merged `13aaaea`).** User: "the modal is not centered." Root cause: TaskDetailModal renders inside MyTasksCard → DashboardWidgetFrame → SortableWidget → WorkspacePanel; WorkspacePanel applies a `transform` for the carousel page transition (PR #81). A transformed ancestor becomes the containing block for any `position: fixed` descendant, so the modal's `fixed inset-0` was anchoring to WorkspacePanel rather than the viewport. Fix: render the overlay through `createPortal(overlay, document.body)`. Same fix PR #72 applied to `SelfReportModal` + `NotificationsBell` for the same reason (`backdrop-filter` also creates a containing block for fixed descendants). Also dropped the doubled `max-h-[90dvh] max-h-[90vh]` declaration; single `max-h-[90dvh]` (Chrome 90+ / Firefox 81+ / Safari 16+). **`CLAUDE:`** authored.
- **PR #85 — Member request actions: unified Delete + Transfer + shared compact composer — 2026-05-02 (merged `3fffc6f`, supersedes #83).** Combines what was originally split across PR #83 (request-to-delete) and the original PR #85 scope (request-to-transfer) into one PR after user feedback that the two flows should look + feel identical and the original transfer composer pushed Send below the modal viewport. Entry points are small **PILL BUTTONS** (icon + label): rose `🗑 Delete` + gold `⇄ Transfer`. Both shown only on the user's own active member-scope tasks. Two-click flow: pill → composer → Send. Shared `<ComposerShell />` atom (single-line header + body slot + Cancel + filled tonal Send + ⌘/Ctrl+Enter / Esc keyboard hint inline). Single state machine (`compose: null | 'transfer' | 'delete'`). Schemas: `task_requests.kind` + `target_task_id` (PR #83 schema) + `task_reassign_requests.direction` (transfer schema) — both ship together. RPCs: `submit_task_delete_request`, `submit_task_transfer_request`, dispatching `approve_task_request` + `approve_task_reassignment` + `decline_task_reassignment`. PR #83 closed as superseded; branch lives on origin for archive. **`CLAUDE:`** authored.
- **PR #84 — AssignAdmin admin-delete UX polish — 2026-05-02 (merged `56693de`).** Two follow-ups to PR #82 after the user tested on production. (1) "Doubling box": in select mode every row was rendering BOTH the rose select-checkbox AND the gold complete-toggle side by side. Fix: in select mode, only the rose select-checkbox renders. (2) Per-row Trash inline confirm: replaced `window.confirm(...)` with the same inline two-step "Confirm delete" pill the bulk Delete uses; auto-resets after 4s; auto-clears if admin enters bulk select mode mid-confirm. **`CLAUDE:`** authored.
- **PR #83 — Member request-to-delete flow — 2026-05-02 (closed; superseded by #85).** Second half of the locked admin-immediate / member-request model (PR #82 shipped admin direct-delete; this adds member-request). Members open the existing TaskDetailModal (already opens via row body click), see a new "Request delete" footer button (rose Trash, only on their own active member-scope tasks), click → footer swaps to a 2-row reason textarea + Send/Cancel; admins see the request in the existing PendingTaskRequestsWidget queue with kind-specific chrome and approve via a rose "Approve delete" CTA that fires the actual delete server-side. **Schema** (`MCP-APPLIED:` migration `task_requests_kind_target`): `task_requests` extended with `kind text NOT NULL DEFAULT 'create' CHECK (IN ('create','edit','delete'))` + `target_task_id uuid REFERENCES assigned_tasks(id) ON DELETE CASCADE` + a kind-target CHECK enforcing `create⇒NULL`, `edit/delete⇒NOT NULL`. Existing rows backfill via DEFAULT. Partial index `(status, kind, created_at DESC) WHERE status='pending'`. **Backend** (`MCP-APPLIED:` migrations `task_delete_request_rpcs` + `approve_task_request_consolidate_dispatch`): (1) `submit_task_delete_request(p_task_id, p_reason?)` — SECURITY DEFINER, member-only (no admin gate), team-scoped lookup, scope='member' + assigned_to=caller required, snapshots task title into `task_requests.title` so the queue still reads after target deletion (CASCADE clears target_task_id), p_reason → description, notifies all team admins via `task_request_submitted`. EXECUTE locked to authenticated. (2) Consolidated `approve_task_request(p_request_id, p_category)` — drops the legacy 1-arg overload (PostgREST was ambiguously resolving against it; client uses 2-arg) and dispatches on kind: `create` runs PR #17 materialize logic verbatim; `delete` deletes target_task_id (defensive: zero-deleted success if CASCADE already cleared); `edit` raises 'unsupported' (next PR). Common tail marks request approved + kind-aware notification body ("Your delete request was approved" vs "Your task request was approved"). (3) `get_pending_task_requests` + `get_my_task_requests` + `admin_recent_approvals` extended to surface `kind` (and target_task_id on first two) in JSON projections. `ADVISOR-VERIFIED:` 0 ERRORS post-migration. **Frontend** ([taskRequests.ts](src/lib/queries/taskRequests.ts) + [adminLogs.ts](src/lib/queries/adminLogs.ts)): `TaskRequestKind` type added; `PendingTaskRequest` + `MyTaskRequest` + `RecentApprovalRow` interfaces gain `kind`; new `submitTaskDeleteRequest(taskId, reason?)` helper. **UI**: (a) [TaskDetailModal.tsx](src/components/tasks/TaskDetailModal.tsx) — "Request delete" button shown only when `!done && scope==='member' && assigned_to===profile.id`; click swaps the footer to an inline rose-tinted reason form (textarea, Send/Cancel) instead of stacking a second modal; on send invalidates `taskRequestKeys.mine()` + `.pending()` + closes detail modal + toasts. (b) [PendingTaskRequestsWidget.tsx](src/components/admin/assign/PendingTaskRequestsWidget.tsx) — rose Trash2 icon + rose accent + "Delete" badge on `kind='delete'` rows; description renders italic with "Reason: " prefix or "No reason provided" italic if empty; approve flow skips FlywheelStagePicker on delete (no relevance) and shows a rose explanatory line; approve button morphs to rose-filled "Approve delete" with shadow; approve mutation passes `null` for category on delete; toast copy is kind-aware (`Deleted — "X" removed from Y's tasks`). invalidate() now also hits `['admin-assigned-tasks']` + `['assign-page-member-tasks']`. (c) [AdminApprovalLogWidget.tsx](src/components/admin/assign/AdminApprovalLogWidget.tsx) — approved-delete rows render a rose Trash2 icon + small uppercase "Deleted" tag inline before the title, distinct from approved-create (emerald check) and rejected (rose X). **Out of scope (next PR)**: edit-request flow — extends `kind='edit'`, adds member edit-request modal with proposed-vs-current diff input, admin diff renderer in PendingTaskRequestsWidget. **Decision sized as "lean" by user**: ship delete-request first end-to-end so the whole approval pipeline is exercised before layering edit on top. **Verified**: `npm run build` clean (2.66s); 0 advisor ERRORS; only pre-existing Supabase auth errors in dev preview console; no JS errors from this PR. **`CLAUDE:`** authored.
- **PR #82 — Admin direct-delete for assigned tasks — 2026-05-02 (merged `52954df`).** User pivoted off Lean 4 mid-session: "lets first do a assign and tasts polish. we really need to get the edit delete tasts function going asap." Delete was the gap — Edit per-row already exists via the pencil → SingleTaskEditModal → `admin_update_assigned_task` (PR #40). Locked model: **admin direct delete = immediate**; **member-side delete = request-to-delete approval flow** (queued as the next PR; see Member-side request flow note below). **Backend** (`MCP-APPLIED:` migration `admin_delete_assigned_tasks`, captured at `supabase/migrations/20260502120000_admin_delete_assigned_tasks.sql`): `admin_delete_assigned_tasks(p_task_ids uuid[])` returns `{ deleted_count }`. SECURITY DEFINER, admin-guarded via `is_team_admin()`, team-scoped via `team_id = get_my_team_id()`, search_path locked to `public`, EXECUTE locked to `authenticated` only. Single bulk RPC takes uuid[] so per-row (1) and bulk (N) hit the same path — same shape as PR #76's hardened admin write/delete RPCs. Cascade verified before writing: `task_reassign_requests.task_id` → CASCADE; `task_requests.approved_task_id` → SET NULL; no FKs from notifications/batches/recipients to `assigned_tasks`. **No notification side effect** in this RPC — admin direct-delete is the explicit choice; if member-side wants delete, that's the request flow's job. `ADVISOR-VERIFIED:` 0 ERRORS, only the by-design `authenticated_security_definer_function_executable` WARN that every SECURITY DEFINER RPC granted to authenticated triggers (matches the 68-warn baseline shape). **Frontend helper** ([adminTasks.ts](src/lib/queries/adminTasks.ts)): new `adminDeleteAssignedTasks(taskIds: string[])` helper that no-ops on empty array. **UI** ([AssignAdmin.tsx](src/pages/admin/AssignAdmin.tsx)): (1) per-row hover-only Trash icon (rose hover, opacity-0 → opacity-100 group-hover) opens `window.confirm("Delete \"X\"? This can't be undone.")` and fires the mutation; (2) new "Select" toggle button in the top action bar (between Save as Template and Templates ▾, disabled when no member or zero tasks); selecting morphs the top bar to `N selected · Select all/Clear all · Delete · Cancel` with a two-step confirm on Delete — first click flashes the button into rose-filled "Confirm delete N" state with the same shadow as `AdminEditSessionsModal.tsx`'s delete CTA, auto-resets after 4s if no follow-up; (3) selected rows get a rose-tinted bg + ring; (4) bulk select-checkbox renders to the LEFT of the existing complete-toggle so the eye lands on what's checked first; (5) per-row Edit + Trash hide in selectMode so the bulk bar is the one place the admin acts from; (6) per locked-decision #8 the bulk action is scoped to the currently-viewed member only (selectMode + selectedIds reset on member change via `useEffect` keyed on `selectedMember?.id`). On success the mutation invalidates `['assign-page-member-tasks', memberId]` + `['assigned-tasks']` + `['admin-assigned-tasks']` + `['team-assigned-tasks']` + `['studio-assigned-tasks']` so member-side widgets reflect deletes immediately. **Out of scope this PR** (deferred): `/daily` admin Trash on Studio + Team Board task rows — those rows are `<button>` click-toggle with hover-overlay for request-to-take; adding admin Trash there clashes with both interactions and would need its own overlay treatment. Admins can canonically delete via /admin/templates which is the central admin task editor. **Member-side request flow (next PR sketch)**: extend `task_requests.kind` enum to `'create' | 'edit' | 'delete'` (or sibling tables matching the existing one-table-per-type pattern with `task_reassign_requests`); members see "Request edit" + "Request delete" buttons where admins see Edit + Trash; PendingTaskRequestsWidget + ApprovalLogWidget extend to render the new kinds; on approve, fire actual update or delete; on decline, optional note (matches existing pattern). **Verified**: `npm run build` clean (2.74s); /admin/templates renders the new Select button correctly at desktop; only pre-existing Supabase auth errors in dev preview console (no JS errors from this PR). **`CLAUDE:`** authored.
