...
- **PR #47 — Assign widget compacted to match Edit — 2026-04-25 evening (in flight).** Pure visual cleanup. User asked for the Assign widget (col 2 top, was rs 1) to match the Edit widget's compact height (col 1 middle, rs 0.5) so they read as a consistent twin-button pair and Task Requests / Approval Log can stack tighter against the top of col 1. Two changes: (1) `AdminAssignWidget` rowSpan in the registry: 1 → 0.5. (2) `AssignTile` body collapsed from a large icon-with-hint card (icon at top + bold label + hint paragraph) to a twin-button row (small icon + bold label inline) — matches `EditButton` exactly. `WORKSPACE_LAYOUT_VERSION` 24 → 25. No DB / RPC / type changes; pure layout + JSX edit. Verified: `tsc --noEmit` clean, `npm run build` 2.67s.
