...
- **WorkspacePanel rebuild**: identical pattern to PR #199 — `expandedId` state, `packedLayout` from the helper. **Expanded widget fills the ENTIRE carousel page** (per user follow-up "use the exact same method as the activity log") — weight = `pageSize` (not a fixed 2), width = `100%`. On desktop where pageSize=3 that's 3-slots-worth = full page; pushed-aside widgets go to a new carousel page so arrow buttons + page dots appear → user can navigate to them. That's the same "carousel feels alive" UX the user signed off on for Activity. Same smooth 320ms easing, same subtle gold ring + drop-shadow. `FloatingDetailModal` import + usage dropped from WorkspacePanel entirely.
