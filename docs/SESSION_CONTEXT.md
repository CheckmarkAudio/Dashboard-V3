...
- **PR #60 — Remove all page-header subtitles — 2026-04-29 (in flight).** User asked for a "modern, simple, sleek" pass on the title pages. Removed the descriptive subtext under every page's `<h1>` — 11 PageHeader callers got their `subtitle` prop deleted (Hub · Templates · TeamManager · TemplateLibrary · ClientsAdmin · Sessions · DailyNotes · Dashboard · DailyChecklist · Pipeline · Leads); 4 inline `<p className="text-text-muted mt-1">` page-header subtitles deleted (AdminSettings · AssignAdmin · Reviews · Schedule). PageHeader component itself untouched — subtitle prop still supported for any future use. Login subtitle preserved (helps unauthenticated users orient). Verified Settings, Assign, Overview in preview — all read as just the title now.
