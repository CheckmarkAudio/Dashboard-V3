...
- **PR #79 — Lean 2: theme + body gradient — 2026-05-01 (in flight).** Two pieces in one CSS pass. (1) Light-mode neutrals shifted from warm cream (`#f6f5f1` / `#f0efea` / `#e7e5dc` / `#d9d7cf` / `#b6b3a6`) to cool grey (`#f4f4f6` / `#ededf0` / `#e3e3e8` / `#d6d6db` / `#afafb6`) so the page reads as grey/gold/black instead of cream/gold/black. Pure white kept on `--color-surface` so cards still pop. (2) Body-level gradient on BOTH themes — gold radial top-left (existing) + new violet radial bottom-right (forum-violet `rgba(139, 92, 246, …)`); dark mode at 0.10/0.05 alpha, light mode at 0.06/0.03 alpha. Subtle by design — locked decision #6 ("body-level, very subtle, NOT on cards") honored: every card surface verified flat (`backgroundImage: none`) via DOM inspection. Tokens-only change; no component edits. **`CLAUDE:`** authored.
