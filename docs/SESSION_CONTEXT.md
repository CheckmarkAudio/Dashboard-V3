...
- **PR #61 — Members > Clock Data shift log — 2026-04-29 (in flight).** Fills the "Clock Data" rail item PR #58 reserved (and that PR #50 deferred). Backend: migration `20260429120000_admin_list_clock_entries.sql` adds one new SECURITY DEFINER RPC `admin_list_clock_entries(p_member_id uuid DEFAULT NULL, p_limit int DEFAULT 100)` returning recent shifts (entry_id · member_id · member_name · clocked_in_at · clocked_out_at · duration_minutes computed at query time · notes), admin-guarded via `is_team_admin()` + scoped to `get_my_team_id()`, sorted clocked_in_at DESC, hard cap 500. No schema changes — reuses the PR #50 `time_clock_entries` table + RLS. Frontend: new `fetchClockEntries(memberId, limit)` wrapper + `entries(memberId)` cache key in `src/lib/queries/timeClock.ts`. TeamManager's `clock-data` section (was a placeholder) now renders a 5-column table inside the Settings-pattern right-pane card: Member (avatar + display_name → linked to `/profile/:id`) · Clock In ("Apr 29, 2026 · 9:14 AM") · Clock Out (timestamp OR emerald "ON SHIFT" pulse pill when open) · Duration ("5m" / "1h 23m" / "8h" / "1d 4h" / "—" for open) · Notes. Member filter dropdown above the table reuses the existing roster (no second fetch). 60s refetchInterval + window-focus refetch so a clock-out from another tab updates this view. Notes column surfaces the "what went well / improve" reflections the clock-out modal collects (per `project_clock_inout_spec.md`). Column model is CSV-friendly for the upcoming `<ExportButtons />` work. Verified: `tsc --noEmit` clean.
