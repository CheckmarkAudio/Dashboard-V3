...
- **PR #49 — Add Member with email-setup flow — 2026-04-29 evening (in flight).** First Tier 2 onboarding slice. Closes the "share a temp password verbally" awkwardness without adding any new email infrastructure — uses Supabase's native recovery-email flow that already exists.
  - **Form change** (`TeamManager.tsx`): `MemberForm` gains `send_setup_email: boolean` (default `true`). Step 1 of the Add-Member modal renders a checkbox card "Email a setup link to the new member" (Mail icon, gold accent, gray hint that flips between "they'll get a secure link at <email>" and "you'll set a temp password below"). When unchecked, the temp-password input returns. `validateStep1` only enforces the 8-char password rule when the toggle is OFF.
  - **Edge function change** (`admin-create-member`, redeployed as v6): `default_password` is now optional. When omitted, a `generateRandomPassword()` helper produces a 32-char crypto-random string (Deno `crypto.getRandomValues` over a curated alphabet — no I/0 or l/1 ambiguity, includes safe symbols). Returns `password_was_generated: boolean` so the client knows whether to trigger the recovery email afterwards. Random plaintext is never returned to the client.
  - **Post-create wire** (`handleSubmit`): when `send_setup_email` is true, after the edge function resolves and templates are assigned, the client fires `supabase.auth.resetPasswordForEmail(email, { redirectTo })` → Supabase sends its native recovery email → the new member clicks the single-use link → lands on the existing `RecoveryGate` → picks their own password. Toast distinguishes the two paths. Failure of the reset email is non-fatal (the member account exists; admin can re-send via `AccountAccessPanel`).
  - **Step-3 review screen** (`ReviewSummary`): when generated, the password row is replaced with `Setup → Email link to member`. The intro paragraph also flips between the two paths.
  - **Drift-fix migration** (`20260426010000_owner_management_rpcs_checkpoint.sql`): captures the existing `owner_set_member_role` + `owner_reset_member_password` RPCs verbatim from prod. They were live in production but never tracked in the migrations folder; capturing them so a fresh checkout can reproduce the schema. Refined to reference `public.team_members` instead of the `intern_users` compat view (functionally equivalent — the view is just `SELECT *` over the table). No behavior change. Applied to staging Supabase.
  - Verified: `tsc -b` clean, `npm run build` 2.78s, edge function v6 ACTIVE on staging.
