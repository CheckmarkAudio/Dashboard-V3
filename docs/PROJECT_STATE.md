...
| **Currently active** | **PR #47 in flight on `claude/pr47-assign-shrink`.** Pure visual cleanup, four revs. (rev1) Assign widget rowSpan 1 → 0.5 + AssignTile collapsed to twin-button row matching Edit. (rev2) Assign-page default order matches user's drag — col 1: Assign Log → Approval Log; col 2: Assign → Edit → Task Requests; col 3: Templates. (rev3) Overview-page defaults match user's drag — My Tasks rs1 → rs2; Booking moved col 2 → col 3 (top); Calendar fills col 2 alone; Notifications rs2 → rs1. (rev4) Added `1.5` to `WidgetRowSpan` union; bumped Notifications rs1 → rs1.5 so col 3 (170 + 16 + 518 = 704px) sits near-flush with cols 1-2 at rs2 (696px). `WORKSPACE_LAYOUT_VERSION` 24 → 28. No DB / RPC changes (one tiny additive type-union change). After this lands the Assign-page redesign is fully shipped — next strategic work is Tier 2 operational maturity (clock-in/out, EmailJS reminders, mark-all-read) and the Cloudflare migration. |
