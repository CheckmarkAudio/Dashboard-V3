...
| **Currently active** | **PR #46 in flight on `claude/pr46-templates-preview`.** Closes out the user-sketched Assign-page redesign. Templates widget gains an Arrange-by sort (A–Z / Newest / Role — Role groups under section dividers); the existing "Include archived" toggle is renamed to "Show archived" per sketch. New `admin_template_preview` widget slots into Assign col 3 directly under Templates: file-system-style 3-col thumbnail grid with role-tag dividers (Engineer / Marketing / Intern / Dev / Admin / Ops / unknown / "No role"), each thumbnail clickable into the same `TemplatePreviewModal` the Templates list uses. `WORKSPACE_LAYOUT_VERSION` 22 → 23. After this lands the Assign-page sketch is fully shipped — next strategic work is Tier 2 operational maturity (clock-in/out, EmailJS reminders, mark-all-read) and the Cloudflare migration. |
...
| 2026-04-25 | (PR #46, in flight) | **Templates enhancements + Preview widget.** Closes out the user-sketched Assign-page redesign. `AdminTemplatesWidget`: existing "Include archived" toggle renamed to "Show archived" per sketch; new Arrange-by selector (A–Z / Newest / Role) sits right of the toggles. Role arrangement groups templates under role-tag dividers (Engineer / Marketing / Intern / Dev / Admin / Ops / extras alphabetically / "No role" last). New widget `admin_template_preview` (component `AdminTemplatePreviewWidget`) slots into Assign col 3 directly below Templates — 3-column thumbnail grid grouped under the same role-tag dividers; each tile is a file-icon + name + item-count clickable into the same `TemplatePreviewModal` the Templates list uses. Reads from the same `get_task_template_library` RPC so react-query reuses the cached payload. WORKSPACE_LAYOUT_VERSION 22 → 23. |
...
- 🟡 **Assign (`/admin/templates`) — 6 of 6 PRs landed (PR #46 awaiting merge).** Per the user's hand-drawn sketch the page has been rebuilt from scratch:
  ...
  - **Col 3**: Templates (rs 2) → Preview (file-system thumbnails grouped by role-tag)
  ...
  - Templates: role-tag pills + "Show archived" toggle + Arrange-by selector (A–Z / Newest / Role); Role arrangement groups under role-name dividers.
  - Preview: 3-col thumbnail grid grouped under role-tag dividers; click opens the same TemplatePreviewModal as the Templates list.
