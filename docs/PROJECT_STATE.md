...
| **Latest commit (main)** | PR #62 in flight — `feat(members): Clock Data shift history table + admin_list_clock_entries RPC`. PR #61 (Overview UI refresh) + PR #50 (Clock In/Out) + PR #58 (Members left-rail) + PR #59 (Assign alignment) + PR #60 (remove subtitles) merged just prior. |
| **Currently active** | **Tier 2 maturity sequence in motion.** PR #62 closes the loop on PR #50: the Members > Clock Data left-rail slot that PR #58 reserved now renders the historical shift table powered by a new `admin_list_clock_entries(p_member_id, p_limit)` RPC. Tier 2 queue still: EmailJS confirmations + 24h reminder + 1hr-after review-ask cron → reusable `<ExportButtons />` → flywheel event ledger. |
| **Prior active** | PR #61 cleaned up the Overview surface (Booking → header button, Notifications full-height, Instagram-style member highlights row, no inline task notes); PR #50 shipped Clock In/Out persistence + Hub "On the Clock" widget; PR #58 restructured Members admin around Settings-style left-rail; PR #60 removed page-header subtitles for the modern/sleek pass. |
