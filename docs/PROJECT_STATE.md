...
| **Latest commit (main)** | `09ec6fc` — feat(assign): PR #10 polish — modal stacking, hard-delete, batch cancel (#10) |
| **Currently active** | **PR #11 open on `claude/assignment-unification-pr11`** — unify the member task surface. (1) **AssignGroupModal rewired** to the new RPC pipeline (`assign_template_to_members` + MemberMultiSelect + `task_templates`). Fixes the "Task Group doesn't notify" bug. (2) **MyTasksCard rewritten** to read `assigned_tasks` directly — mock arrays + `MyTasksContext` retired. "My Tasks" is now an honest, DB-backed surface. (3) **`AssignedTasksWidget` retired** — content folded into `team_tasks` so members see everything in one place. Layout version bumped 9→10. (4) **Click-to-highlight**: Notifications widget's assignment rows dispatch `highlight-task` CustomEvent with `batchId`; MyTasksCard listens, scrolls matching row into view, flashes gold ring for 1.6s. |
| **Prior active** | PR #10 assignment-system polish (merged `09ec6fc`) — modal stacking, hard-delete, batch cancel. |
...
| 2026-04-22 | (PR #11, pending) | **Unify member task surface.** AssignGroupModal (Hub "Task Group" tile) rewired from legacy `task_assignments` insert to `assignTemplateToMembers` RPC — now fires notifications + materializes `assigned_tasks`. MyTasksCard rewritten to read `assigned_tasks` via `fetchMemberAssignedTasks`, optimistic complete via `completeAssignedTask`, realtime on `assigned_tasks` INSERT/UPDATE for the current user. Mock seed arrays + `MyTasksContext` + `CreateTaskModal` deleted (never DB-backed). `AssignedTasksWidget` retired from the registry — its content is now inside `team_tasks` (MyTasksCard). `WORKSPACE_LAYOUT_VERSION` 9→10. Click-to-highlight: Notifications widget's assignment rows dispatch `highlight-task` CustomEvent with `batchId`; MyTasksCard listens, scrolls the first matching task into view, flashes a gold ring for ~1.6s. Same wiring on `AdminNotificationsWidget`. |
