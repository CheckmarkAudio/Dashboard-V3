...
- ⏳ **Phase 1 frontend MVP** (in flight, PR #7). Custom-task assign only (brief's minimum testable slice). New `AssignedTasksWidget` on Overview + Tasks (multi-page placement via `defaultPlacements` in registry). `AssignCustomTaskModal` admin surface on Hub Assign widget (4th tile). Notifications widget extended with Assignments section + realtime. Widget-visibility model formalized (`accessVisibility` + `dataScope`). `/daily` restructured into a widget grid; mock Team/Studio cards retired.
- **Phase 2 (deferred)**: Template library UI, full + partial template assign, update/delete/duplicate template ops, cancel batch, assignment history, `mark_all_read`, fold assigned-tasks summary into `member_overview_snapshot`, unify `report_templates` into `task_templates`, wire real-data Team Tasks / Studio Tasks widgets back onto the Tasks page.
