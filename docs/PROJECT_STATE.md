...
| **Currently active** | **PR #48 in flight on `claude/pr48-mark-all-read`.** First Tier 2 slice. Two new SECURITY DEFINER RPCs — `mark_all_channels_read()` upserts a `chat_channel_reads` row per channel for the caller, `mark_all_assignment_notifications_read()` flips `is_read=true` on every unread row where `recipient_id=auth.uid()`. New "Mark all read" button in both `ForumNotificationsWidget` (member Overview) + `AdminNotificationsWidget` (admin Hub) sits in the eyebrow row beside the unread pill, hidden when nothing is unread. Single click fires both RPCs in parallel and optimistically clears every channel + assignment unread in cache; on error, query keys invalidate to flow server truth back. Migration `20260426000000_mark_all_read_rpcs.sql` applied to staging — Supabase advisors clean (no new warnings). After this, next Tier 2 slices are clock-in/out logging and EmailJS booking/session reminders. |
