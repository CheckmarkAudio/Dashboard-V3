...
| **Latest commit (main)** | `76884fa` — chore(layout): hide workspace controls bar on all 4 pages (PR #31) |
| **Currently active** | **Nothing in flight.** Session of 2026-04-24 shipped PRs #17–#31 (fifteen merges) consolidating the layout + task surfaces of Overview, Tasks, Hub, and Assign. All 4 pages now run on `WorkspacePanel` with a uniform 3-column equal-width grid, drag-to-reorder, and expand-to-modal. Flywheel widget now reads real `assigned_tasks` data with dual-opacity bars (opaque = done, translucent = assigned-open). Task click opens full `TaskDetailModal`. Rich `+Task` modal with flywheel tagging. Unified admin task modal. Controls card hidden site-wide — widgets are non-removable by design. |
| **Prior active** | PRs #11–#16 — Monday-style task system (unified MyTasks, scope model, Assign page redesign, studio-scope writes, session polish, self-serve task requests). |
...
**Progress:**
- ✅ **Overview (member) — complete.** 3-column `WorkspacePanel` grid; Tasks · Today's Calendar · Forum Notifications · Booking Snapshot. Drag-reorder + expand-to-modal. TODAY eyebrow on every widget. Shared `CalendarDayCard` between Overview + `/calendar`.
- ✅ **Admin Hub — complete.** 3-column `WorkspacePanel` grid: Quick Assign + Approvals · Flywheel · Notifications + Team. Flywheel reads real `assigned_tasks` data per stage (dual-opacity done/open bars). Unified Quick Assign modal covers all admin task-assign flows.
- ✅ **Tasks (`/daily`) — complete.** 3-column `WorkspacePanel` grid: My Tasks · Studio Tasks · Team Board. Bottom-of-list "+ Task" add pattern. Click body → `TaskDetailModal`, click checkbox → complete.
- ✅ **Assign (`/admin/templates`) — complete.** 3-column `WorkspacePanel` grid: Assign (tiles) · Task Requests · Templates library. Canonical filter pills with counts; pinned-tile pattern.

All 4 refreshed pages now share the same grammar: `WorkspacePanel` + equal-width 3-column grid + `DashboardWidgetFrame` (drag grip + expand). Controls bar hidden — widgets are visible-by-default + non-removable for now.
