...
| **Currently active** | **PR #46 in flight on `claude/pr46-templates-preview`.** Closes out the user-sketched Assign-page redesign. Templates widget gains an Arrange-by sort (A–Z / Newest / Role — Role groups under section dividers); the existing "Include archived" toggle is renamed to "Show archived" per sketch. After user feedback the thumbnail grid lives INSIDE the Templates widget — search / filters / Arrange row stays pinned at top while a 2-per-row grid of friendly tiles scrolls beneath. Each tile uses a per-role icon (Headphones=engineer, Megaphone=marketing, GraduationCap=intern, Code2=dev, Briefcase=admin, Settings=ops). Onboarding templates get a tiny emerald GraduationCap badge on the icon corner. Templates rowSpan stays at 2 so col 3 reads balanced against cols 1-2. `WORKSPACE_LAYOUT_VERSION` 22 → 24 (rev1 + rev2 bumps). After this lands the Assign-page sketch is fully shipped — next strategic work is Tier 2 operational maturity (clock-in/out, EmailJS reminders, mark-all-read) and the Cloudflare migration. |
