...
| **Latest commit (main)** | PR #60 in flight — `chore(ui): remove page-header subtitles across the app — modern/sleek pass`. PR #59 (Assign right-pane alignment fix) merged just prior; PR #58 (Members left-rail restructure) also open. |
| **Currently active** | **Assign-page redesign (member-centric editor).** Per the [project_assign_redesign_priority memory](~/.claude/projects/-Users-bridges-GITHUB-Dashboard-V3/memory/project_assign_redesign_priority.md), this is THE priority — boss-driven member-centric task editor where you pick a member from a left rail and edit their full task list on the right. PRs #52–#57 rebuilt the Assign page from scratch around that pattern: visual scaffold (#52, draft) → real-data wire-up + checkbox flicker fix + template preview-before-apply (#53) → legacy widget-grid view isolated to `/admin/assign-classic` with sidebar shortcut (#54, #55) → Templates manager extracted to its own full page at `/admin/template-library` (#56) → page header `<h1>Assign</h1>` + subtitle added to align with Settings page rhythm (#57). Tier 2 PRs (clock-in/out persistence #50, EmailJS reminders) are explicitly paused until the Assign redesign settles. |
| **Prior active** | PR #48 mark-all-read (merged) was the last Tier 2 slice before the Assign redesign took priority. |
