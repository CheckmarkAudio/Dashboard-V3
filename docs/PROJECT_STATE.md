...
| **Latest commit (main)** | `8c863dc` — feat(assign): Approval Log widget + admin_recent_approvals RPC (PR #45) |
| **Currently active** | **PR #46 queued, nothing in flight.** Session of 2026-04-25 shipped PRs #32–#45 across two extended sittings — fourteen merges that completed the task system + landed the user-sketched Assign-page redesign. Layout-interaction overhaul (#32-#34): column-snap grid with `DragOverlay` ghost, direct 1-for-1 swap on cross-column drops. Task UI polish (#35-#36): no double-box on /daily widgets, flywheel stage pills + right-aligned due dates, footer cleanup. Task system fixes (#37-#38): pending→Submit Completed pattern restored, `can_complete` RPC bugs fixed, peer "Request to take" reassignment with approve/decline modal. Tier 1 cleanup (#39): studio-task tile, admin-hub reassign parity, approve race safety. Edit Tasks library (#40). **Assign-page redesign per user sketch (#41-#45)**: 3-col reorg, +Task modal becomes row-by-row with Members/Studio toggle + Add-from-template sub-flow, Edit twin-button (Edit Task + Edit Booking) + Edit Bookings backend/modal, Assign Log + Approval Log widgets. **Next: PR #46 — Templates enhancements (role-tag pills, Show-archived, Arrange-by) + new Preview widget grouped by role-tag with file-system-style thumbnails.** |
...
- 🟡 **Assign (`/admin/templates`) — 5 of 6 PRs landed.** Per the user's hand-drawn sketch the page has been rebuilt from scratch:
  ...
  - **Col 3**: Templates (rs 2) → **Preview widget queued for PR #46**
...
**PR #46 (queued, last piece of the Assign-page redesign):** Templates enhancements — role-tag pills, "Show archived" toggle, "Arrange by" sort (alphabetical / date added / role) — plus a new Preview widget below Templates: clickable file-system-style thumbnails grouped under role-tag dividers. Thumbnails open the same template detail modal as the Templates list.
