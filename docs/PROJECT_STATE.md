...
| **Latest commit (main)** | `c0e6a47` — feat(tasks): user task requests + admin approval queue (PR #16) |
| **Currently active** | **Nothing in flight.** Session of 2026-04-22 shipped PRs #11–#16 (six merges) taking the task/assignment system from MVP to near-complete. Monday-style task system is live: three-column `/daily` (My · Studio · Team), redesigned Assign page with Quick Assign + session-assign + templates, studio-scope write-path, session click-to-highlight + conflict warnings, self-serve task requests with admin approval queue. Also shipped: preview auto-login (skips login form on PR branches). |
| **Prior active** | PR #10 assignment-system polish — modal stacking, hard-delete, batch cancel. |
