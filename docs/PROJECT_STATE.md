...
| **Latest commit (main)** | `3a4ade7` — feat(assignments): Phase 2 backend — edit ops + batch cancel + preview (#8) |
| **Currently active** | **PR #9 open on `claude/assign-page-redesign`** — comprehensive `/admin/templates` Assign-page redesign. Full replacement of legacy Templates.tsx (1069 → 360 lines on the page itself; ~2000+ net new lines across 8 new files). Ships: filter bar (search + role pills + onboarding/archived toggles) + responsive card grid + 4 modals (Preview / Editor / Duplicate / 3-step Assign wizard) + extracted `MemberMultiSelect` shared between Hub and new Assign wizard. Legacy `report_templates` direct-CRUD surface retired; daily-checklist admin stays on `PublishChecklistModal`. **Known issue**: nested modals share `z-[60]` so stacked modals (preview + editor/duplicate/assign) have overlapping click targets — to be fixed in PR #10. |
| **Prior active** | Phase 2 backend prep (PR #8, merged `3a4ade7`). Shipped 6 admin RPCs (`update_task_template`, `update_task_template_item`, `delete_task_template_item`, `duplicate_task_template`, `cancel_task_assignment_batch`, `assign_template_preview`) + FK `ON DELETE SET NULL` on `assigned_tasks` → `task_templates(_items)` + `ar.status = 'active'` filter on `get_member_assigned_tasks`. |
...
- ⏳ **Assign-page redesign** (in flight, PR #9). Full `/admin/templates` rewrite targeting `task_templates`. Card grid + filter + 4 modals (Preview / Editor / Duplicate / 3-step Assign wizard). Known issue: nested modal stacking queued for PR #10.
- ⏳ **PR #10 (next)**: modal stability fix (elevate sub-modal z-index to `z-[70]`, scope Escape to topmost modal, breadcrumb in sub-modal eyebrow) + batch cancel UI surfacing `cancel_task_assignment_batch` RPC on the Admin Hub "Recently assigned" strip.
