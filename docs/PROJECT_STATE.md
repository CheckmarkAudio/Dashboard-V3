...
| **Latest commit (main)** | `6b23dbb` — feat(assign): Templates Arrange-by + Show-archived + Preview widget (PR #46) |
| **Currently active** | **PR #47 in flight on `claude/pr47-assign-shrink`.** Pure visual cleanup, three revs. (rev1) Assign widget rowSpan 1 → 0.5 + AssignTile collapsed to twin-button row matching Edit. (rev2) Assign-page default widget order matches user's drag layout — col 1: Assign Log → Approval Log; col 2: Assign → Edit → Task Requests; col 3: Templates. (rev3) Overview-page defaults updated to match user's drag — My Tasks rs1 → rs2 (matches Calendar height); Booking moved col 2 → col 3 (top); Calendar fills col 2 alone; Notifications rs2 → rs1 so col 3 (Booking rs0.5 + Notifications rs1) reads as a balanced stack. `WORKSPACE_LAYOUT_VERSION` 24 → 27. No DB / RPC / type changes. After this lands the Assign-page redesign is fully shipped — next strategic work is Tier 2 operational maturity (clock-in/out, EmailJS reminders, mark-all-read) and the Cloudflare migration. |
...
| 2026-04-25 | (PR #47, in flight) | **Assign + Overview default widget orders updated.** Pure visual cleanup, three revs. (rev1) `AdminAssignWidget` rowSpan 1 → 0.5 + `AssignTile` collapsed to twin-button row matching `EditButton`. (rev2) Assign-page placements per user drag: col 1 = Assign Log → Approval Log; col 2 = Assign (rs0.5) → Edit (rs0.5) → Task Requests; col 3 = Templates (rs2). Reordered `ADMIN_WIDGET_REGISTRATIONS` array so within-column order resolves. Hub overview placements unchanged. (rev3) Overview-page placements per user drag: `team_tasks` rs1 → rs2 to match Calendar height; `booking_snapshot` col 2 → col 3 (top, rs0.5 unchanged); `today_calendar` fills col 2 alone (rs2 unchanged); `forum_notifications` rs2 → rs1 so col 3 reads as a balanced stack. Tasks-page placements unchanged. `WORKSPACE_LAYOUT_VERSION` 24 → 27. |
