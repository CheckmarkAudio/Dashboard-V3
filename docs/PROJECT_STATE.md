...
| **Latest commit (main)** | `8821785` — feat(assignments): Phase 1 frontend MVP — custom-task flow end-to-end (#7) |
| **Currently active** | **Phase 2 backend prep PR #8** shipping now: 6 admin RPCs (update/delete/duplicate template ops, cancel batch, preview) + FK `ON DELETE SET NULL` on `assigned_tasks` → `task_templates(_items)` so template evolution never breaks existing team progress. `get_member_assigned_tasks` gains an `ar.status = 'active'` filter so cancelled batches disappear from member view immediately. Next after merge: Assign-page (`/admin/templates`) comprehensive redesign, then circle back to the Hub Assign widget to snapshot. |
| **Prior active** | Assignment-system **frontend MVP** (PR #7). Adds `AssignedTasksWidget` (personal/self widget on both Overview + Tasks), extends Notifications widget with an Assignments section (two RPCs + dual realtime), adds a 4th "Custom Task" tile to the Admin Hub Assign widget with a new `AssignCustomTaskModal` → `assign_custom_task_to_members` RPC. Formalizes widget visibility model (`accessVisibility` + `dataScope` on registrations, locked via disjoint Member/Admin scope types). Tasks page (`/daily`) restructured from a 3-card hand-composed layout into a widget grid; mock `TeamTasksCard` and `StudioTasksCard` retired. Layout version bumped 8 → 9. |
...
- ⏳ **Phase 2 backend prep** (in flight, PR #8). 6 admin-only RPCs for template editing + batch cancel + assignment preview. `ON DELETE SET NULL` FK tweak on `assigned_tasks → task_templates(_items)` so template evolution cannot destroy historical team progress. `get_member_assigned_tasks` gains `ar.status = 'active'` filter so cancelled batches vanish from member view. No frontend; ships the schema the comprehensive Assign-page UI will call.
- ⏳ **Next: comprehensive Assign page (`/admin/templates`) redesign** — mockup-style template library (list + quick filter + preview pane + per-template task CRUD + assign flow). After that lands, circle back to the Admin Hub Assign widget to decide what snapshot subset belongs there.
- **Phase 3+ (deferred)**: assignment history UI, `mark_all_read`, fold assigned-tasks summary into `member_overview_snapshot`, unify `report_templates` into `task_templates`, wire real-data Team Tasks / Studio Tasks widgets back onto the Tasks page.
