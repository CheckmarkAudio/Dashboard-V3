...
| **Latest commit (main)** | `90ce0bf` — docs: reflect 2026-04-25 session (PRs #35-#45 merged) + PR #46 queued |
| **Currently active** | **PR #46 in flight on `claude/pr46-templates-preview`.** Closes out the user-sketched Assign-page redesign. Templates widget gains an Arrange-by sort (A–Z / Newest / Role — Role groups under section dividers); the existing "Include archived" toggle is renamed to "Show archived" per sketch. After user feedback on the first pass, the file-system-style thumbnail grid was folded directly into the Templates widget (replacing the big preview cards) instead of sitting in a separate `admin_template_preview` widget — the search / filters / Arrange row stays pinned at top while the 3-per-row thumbnail grid scrolls beneath. Templates rowSpan 2 → 3 so it fills col 3. `WORKSPACE_LAYOUT_VERSION` 22 → 23. After this lands the Assign-page sketch is fully shipped — next strategic work is Tier 2 operational maturity (clock-in/out, EmailJS reminders, mark-all-read) and the Cloudflare migration. |
| **Prior active** | PRs #17–#31 — Layout + task-surface consolidation that established the column-snap grammar across all 4 refreshed pages. |
...
| 2026-04-25 | (PR #46, in flight) | **Templates Arrange-by + file-system thumbnails.** Closes out the user-sketched Assign-page redesign. `AdminTemplatesWidget`: existing "Include archived" toggle renamed to "Show archived" per sketch; new Arrange-by selector (A–Z / Newest / Role) sits right of the toggles; Role arrangement groups templates under role-tag dividers (Engineer / Marketing / Intern / Dev / Admin / Ops / extras alphabetically / "No role" last). The big-card grid is replaced with file-system-style thumbnails — small uniform tiles (file icon + name + item count) in a 3-per-row grid. The search / filters / Arrange-by row stays pinned at top so controls are always visible while the thumbnail grid scrolls. Each thumbnail opens the same `TemplatePreviewModal` the cards used. First pass also shipped a separate `admin_template_preview` widget below Templates, but the user asked for the thumbnails inside Templates instead — so that widget + its file (`AdminTemplatePreviewWidget.tsx`) + the now-unused `TemplateCard.tsx` were deleted, and Templates' rowSpan bumped 2 → 3 to fill col 3. WORKSPACE_LAYOUT_VERSION 22 → 23. |
...
- 🟡 **Assign (`/admin/templates`) — 6 of 6 PRs landed (PR #46 awaiting merge).** Per the user's hand-drawn sketch the page has been rebuilt from scratch:
  ...
  - Templates: search + role-tag pills + Onboarding/Show-archived toggles + Arrange-by selector (A–Z / Newest / Role) all pinned at top; 3-per-row file-system-style thumbnail grid scrolls beneath. Role arrangement adds role-name dividers above each section. Each thumbnail opens the shared TemplatePreviewModal.
