...
| **Latest commit (main)** | PR #61 in flight — `feat(overview): Booking → header button, Notifications full-height, member highlights row, drop inline task description`. PR #50 (Clock In/Out) + PR #58 (Members left-rail) + PR #59 (Assign alignment) + PR #60 (remove subtitles) merged just prior. |
| **Currently active** | **Overview UI refresh + Tier 2 sequence still in motion.** PR #61 cleans up the Overview surface (booking moves to a header button, Notifications stretches to match col 1-2, member-avatar highlight row added Instagram-style, task-row description killed since notes belong only in the expanded modal). Tier 2 queue still: populate Members Clock Data pane with shift history → EmailJS confirmations + 24h reminder + 1hr-after review-ask cron → reusable `<ExportButtons />` → flywheel event ledger. |
| **Prior active** | PR #50 shipped Clock In/Out persistence + Hub "On the Clock" widget; PR #58 restructured Members admin around Settings-style left-rail; PR #60 removed page-header subtitles for the modern/sleek pass; PR #59 fixed Assign right-pane alignment. |
