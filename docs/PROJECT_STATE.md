...
| **Currently active** | **PR #47 in flight on `claude/pr47-assign-shrink`.** Pure visual cleanup. (rev1) Assign widget rowSpan 1 → 0.5 so it matches the Edit widget's compact height; tile body collapsed from large icon-with-hint cards to a twin-button row (icon + label) matching Edit's button style. (rev2) Default Assign-page widget order updated to match the user's drag layout — col 1: Assign Log → Approval Log; col 2: Assign → Edit → Task Requests; col 3: Templates. `WORKSPACE_LAYOUT_VERSION` 24 → 26. No DB / RPC / type changes. After this lands the Assign-page redesign is fully shipped — next strategic work is Tier 2 operational maturity (clock-in/out, EmailJS reminders, mark-all-read) and the Cloudflare migration. |
...
| 2026-04-25 | (PR #47, in flight) | **Assign widget compacted + new default Assign-page widget order.** Pure visual cleanup. (rev1) `AdminAssignWidget` rowSpan 1 → 0.5 in the registry; `AssignTile` collapsed from a large icon-with-hint card to a twin-button row (icon + label inline) matching `EditButton` exactly. (rev2) Default widget placements updated to match the user's drag layout: col 1 = Assign Log → Approval Log; col 2 = Assign (rs0.5) → Edit (rs0.5) → Task Requests; col 3 = Templates (rs2). Required moving `admin_assign_log` ahead of `admin_approval_log` in the registration array, and moving `admin_edit_tasks` ahead of `admin_task_requests` so within-column order resolves correctly. Hub overview placements unchanged (admin_quick_assign + admin_task_requests still stack in col 1 in that order). `WORKSPACE_LAYOUT_VERSION` 24 → 26. |
