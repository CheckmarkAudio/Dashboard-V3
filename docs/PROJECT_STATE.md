...
| **Latest commit (main)** | PR #58 in flight — `feat(members): left-rail restructure + extract AdminSectionNavItem`. PRs #59 (Assign alignment) + #60 (remove subtitles) merged just prior. |
| **Currently active** | **Tier 2 maturity sequence resumed.** User unblocked everything on 2026-04-29 by answering the batch of Tier 2 questions (clock-in/out spec, EmailJS path, exports, flywheel feels-decorative, future chat/forum direction). Assign-page redesign moved to maintenance — the "Save as Template" + "Settings for Tasks" stubs deferred. Active sequence: **PR #58 (Members left-rail restructure, in flight)** → PR #50 Clock In/Out v2 (resume `claude/pr50-clock-in-out` with expanded scope: shifts table + reflection modal + Clock Data view) → EmailJS confirmations + 24h reminder + 1hr-after review-ask cron → reusable `<ExportButtons />` → flywheel event ledger (Tier 2 PRs write rows as side effects so the widget stops feeling decorative). |
| **Prior active** | PR #60 removed page-header subtitles for the modern/sleek pass; PR #59 fixed Assign right-pane alignment; PR #57 closed out the Assign rebuild PR series (#52–#57). |
