...
| 2026-04-25 | (PR #47, in flight) | **Assign widget compacted to match Edit.** Pure visual cleanup. `AdminAssignWidget` rowSpan 1 → 0.5 in the registry so it matches the Edit widget's compact height; `AssignTile` collapsed from a large icon-with-hint card (icon at top, big label, hint paragraph) to a twin-button row (small icon + bold label inline) matching `EditButton` exactly. Two widgets read as a visually consistent pair on the Assign page, and Task Requests / Approval Log in col 1 stack tighter against the top. `WORKSPACE_LAYOUT_VERSION` 24 → 25. |
