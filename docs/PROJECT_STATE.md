...
| **Latest commit (main)** | `7d9a5a0` — feat(assign): compact Assign widget + new default Assign + Overview widget orders (PR #47) |
| **Currently active** | **PR #49 in flight on `claude/pr49-add-member`.** Tier 2 onboarding slice. Add-Member form gains a "Email a setup link" toggle (default ON) — when on, the edge function generates a 32-char crypto-random password server-side and the client fires `auth.resetPasswordForEmail()` after creation so the new member receives a single-use link to set their own password. Toggle off keeps the manual-share temp-password path for offline / walk-in adds. Tracked migration `20260426010000_owner_management_rpcs_checkpoint.sql` captures the existing `owner_set_member_role` + `owner_reset_member_password` RPCs (drift fix — they live in prod but weren't in the migrations folder). Edge function redeployed (v6). No new email infrastructure — uses Supabase's native recovery email flow. **PR #48** (Mark-all-read) also still awaits your review on `claude/pr48-mark-all-read`. Once both merge: `@dnd-kit` local-build fix → Cloudflare migration. |
| **Prior active** | PRs #17–#31 — Layout + task-surface consolidation that established the column-snap grammar across all 4 refreshed pages. PRs #41–#47 — Assign-page redesign per user sketch + Overview default-layout polish. |
...
|--|--|--|
| 2026-04-29 | (PR #49, in flight) | **Add Member with email-setup flow.** First Tier 2 onboarding slice. `MemberForm` gains `send_setup_email: boolean` (default true). When on, the form hides the password input, the edge function `admin-create-member` (now v6) generates a 32-char crypto-random password server-side, and the client fires `auth.resetPasswordForEmail()` after the create returns — Supabase sends its native recovery email; the new member clicks the single-use link, lands on the `RecoveryGate`, picks their own password, done. When off, the temp-password input returns for manual-share / offline adds. Edge function returns `password_was_generated: boolean` so the client knows when to trigger the reset email. Random plaintext is never returned to the client. Toast distinguishes the two paths. Step-3 review screen omits the password row when generated and adds a "Setup → Email link to member" row instead. Tracked migration `20260426010000_owner_management_rpcs_checkpoint.sql` captures the existing `owner_set_member_role` + `owner_reset_member_password` RPCs (they were live in prod but never tracked — drift fix). |
...
- ✅ **Assign (`/admin/templates`) — sketch redesign fully shipped (PRs #41-#46 merged; #47 in flight as a follow-up polish).** Per the user's hand-drawn sketch the page has been rebuilt from scratch:
  - **Col 1**: Task Requests (top) → Approval Log → Edit twin-button (Edit Task + Edit Booking, rs 0.5)
  - **Col 2**: Assign (2 tiles: +Task / +Booking) → Assign Log
  - **Col 3**: Templates (rs 2 — friendly thumbnails with per-role icons)
  - +Task modal is row-by-row with a Members/Studio toggle + "Add from template" sub-flow that pulls items into editable rows.
  - Edit Task / Edit Booking modals support search, filters, click-to-expand edit forms, optional delete (booking).
  - Assign Log: 3-col rows (title / "First L." / "Today"|"Apr 25") interleaving member tasks + studio pool + sessions.
  - Approval Log: green ✓ / rose ✕ rows with requester + relative time, decline rows show the note inline.
  - Templates: search + role-tag pills + Onboarding/Show-archived toggles + Arrange-by selector (A–Z / Newest / Role) all pinned at top; 2-per-row friendly thumbnail grid scrolls beneath. Each tile uses a per-role icon (Headphones / Megaphone / GraduationCap / Code2 / Briefcase / Settings / FileText fallback). Role arrangement adds role-name dividers above each section. Each thumbnail opens the shared TemplatePreviewModal.
