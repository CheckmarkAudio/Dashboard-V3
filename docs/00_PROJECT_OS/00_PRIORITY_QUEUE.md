...
- [x] Calendar page ("Team Schedule" tab) visual revamp + schedule widgets purple-to-brand-color cleanup — Requested by: director · Added: 2026-07-26/2026-07-28 · Done: 2026-07-28 · Notes: Shipped across [PR #328](https://github.com/CheckmarkAudio/Dashboard-V3/pull/328) (mockups signed off in-chat, then built: member-rows Team Schedule grid, gold-filled toggle pinned to a fixed left position, locked per-member colors, time labels decoupled from bar width so short shifts never clip) and a follow-up PR (schedule-edit modal "current schedule" visual breakdown + finishing the purple-to-gold/per-member-color sweep on `AdminEmployeeScheduleWidget.tsx`, `MyScheduleWidget.tsx`, `WorkScheduler.tsx`, `ProfileWeeklySchedule.tsx` — the two originally-named files plus two more found in the same family while sweeping). **Hour-accuracy** resolved by construction — the grid positions blocks directly from real `starts_at`/`ends_at` using the same percentage-mapping already proven in `HeaderActivityBar`, not a separate/approximate calculation. **Hover-highlight decision** (NEEDS-DIRECTOR, never answered): resolved by staying retired — always-visible per-member color was the explicit goal of this redesign, and reintroducing dim-until-hover would reverse that without a stated reason to. Full history in `docs/ux/SCHEDULE_UX_REDESIGN_PLAN.md`.
